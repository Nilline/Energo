...
let pageSlider = new Swiper('.page', {
	...
	speed: 800,
	// Обновить свайпер
	// при изменении элементов сладера
	observer: true,
	...
	on: {
		...
		slideChange: function () {
			menuSliderRemove();
			sliderLinks[pageSlider.realIndex].classList.add('_active');
			sliderMenu.classList.add('_active')
			const timer = setTimeout(menuSliderRemove, 3000);
			clearTimeout(timer);
			setTimeout(menuSliderRemove, 3000);
		},
		resize: function () {
			setScrollType();
		},
...
let sliderMenu = document.querySelector('.page__menu');
let sliderLinks = document.querySelectorAll('.page__link');

function menuSliderRemove() {
	let sliderLinkActive = document.querySelector('.page__link._active');
	if (sliderLinkActive) {
		sliderLinkActive.classList.remove('_active');
		sliderMenu.classList.remove('_active');
	}
}
...
if (sliderLinks.length > 0) {
	for (let index = 0; index < sliderLinks.length; index++) {
		const pageLink = sliderLinks[index];
